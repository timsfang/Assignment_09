...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        file_name_CD = file_name[0] # assign 1st file name to CD inventory txt file
        file_name_track = file_name[1] # assign 2nd file name to track inventory txt file
        try:
            with open(file_name_CD, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_record()) # code to write CD inventory txt file
            with open(file_name_track, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_tracks()) # code to write track inventory txt file
        ...
        try:
            with open(file_name_CD, 'r') as file:
                for line in file:
                    ...
                    lst_Inventory.append(row) # code to load CD inventory from txt file
            with open(file_name_track, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    row = DC.Track(data[1], data[2], data[3])
                    lst_Inventory.append(row) # code to load track inventory from txt file
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
